...
st.title("🔬 Laboratório de Testes e Diagnóstico")
st.markdown(
    "Central unificada para testar a extração do seu currículo, "
    "inspecionar dados da API e conferir os modelos do Gemini."
)
...
# ==========================================
with tab_cv:
    st.header("📄 Diagnóstico de Currículo (Visão do ATS)")
    st.markdown(
    ...
    if arquivo_pdf and st.button("🔍 Extrair e Simular Leitura do ATS", type="primary"):
        with st.spinner("Convertendo PDF e enviando para a IA de Diagnóstico..."):
            ...
            with col1:
                st.subheader("1. Texto Bruto (Visão do Robô)")
                st.markdown("É **exatamente isto** que a IA recebe (Markdown):")
                st.text_area(
            ...
            with col2:
                st.subheader("2. Interpretação da IA")
                prompt = f"""
...
# ==========================================
with tab_mcp:
    st.header("🕵️ Explorador de Dados Brutos (MCP)")

    col_m1, col_m2 = st.columns(2)

    with col_m1:
        st.subheader("1. Inspecionar Servidor")
    ...
    with col_m2:
        st.subheader("2. Extração Completa")
        funcao_mcp = st.radio(
            "🛠️ Ferramenta MCP:",
    ...
    if st.session_state.get("vagas_mcp"):
        ...
        if "Tags" in visao:
            ...
            st.dataframe(vagas_tab, use_container_width=True)
...
# ==========================================
with tab_modelos:
    st.header("🤖 Explorador de Modelos da API (Google Gemini)")
    if st.button("🔍 Listar Modelos Disponíveis na Minha Conta"):
